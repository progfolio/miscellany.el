...
(defun my/recompile-user-package-dir ()
 ...
 #+end_src
** server-eval-all
I use a number of Emacs of servers daily. When I make changes to my =init.org=, I
want those changes to propagate to all running servers.
#+begin_src emacs-lisp
(defun my/server-eval-all (form)
  "Evaluate FORM on each client of an Emacs server.
FORM is a quoted list of expressions."
  (let* ((files (directory-files server-socket-dir nil nil t))
         (sockets (seq-filter (lambda (file)
                                (not (member file '("." ".."))))
                              files)))
    (mapcar (lambda (socket)
              (message (concat "evaling on " socket))
              (server-eval-at socket form))
            sockets)))
#+end_src
